...
import { getVisibleElements } from '../graph/index.js';
...
/**
 * Clamps `x` and `y` so the legend element stays within its parent's bounds with `CLAMP_MARGIN` px
 * of spacing on every side.
 *
...
function makeDraggable(el) {
  ...
  el.addEventListener('mousedown', (e) => {
    ...
    e.preventDefault();
    e.stopPropagation(); // Prevent canvas tap-deselect
    dragging = true;
    startX = e.clientX;
    startY = e.clientY;
...
export function initLegend() {
  ...
  // Re-clamp on every canvas resize (window resize, sidebar toggle, etc.).
  // Does NOT save the adjusted position — the user's dragged position is preserved in
  // LocalStorage and re-evaluated the next time the element fits.
  const parent = el.parentElement;
  if (parent) {
    new ResizeObserver(() => {
...
/** Initializes legend module: registers global event listeners. */
export function init() {
  document.addEventListener('graph:visibilityApplied', updateLegend);
  document.addEventListener('highlight:legendUpdate', (e) => {
    updateLegendHighlight(e.detail.fadedElemTypes, e.detail.fadedRelTypes);
  });
}
