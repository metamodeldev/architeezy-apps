...
function parseEClass(raw, nsMap) {
...
function nodeName(d, fallback) {
  return d.name || d.label || d.title || fallback;
}

function nodeDoc(d) {
  return d.documentation || d.description || d.doc || '';
}

/**
 * Parses a raw Architeezy model JSON response into `state.allElements`, `state.allRelations`, and
 * `state.elemMap`. Also resolves the model namespace URI into `state.currentModelNs`.
 *
...
/**
 * Recursively walks all array-valued properties of a data object. Objects with `eClass` are
 * dispatched to `walkNode`; plain UUID strings produce implicit edges from `parentId` to the
 * referenced UUID.
 *
...
/**
 * Classifies a single eClass node as a standalone edge, embedded reference edge, or graph element,
 * then recurses into its data.
 *
 * @param {object} node - Raw eClass node from the model JSON.
 * @param {string | undefined} parentId - ID of the containing element, or undefined at root level.
 */
function walkNode(node, parentId) {
  const { ns, type, fullNs } = parseEClass(node.eClass, state.modelNsMap);
