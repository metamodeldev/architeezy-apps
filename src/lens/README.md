...
**No build step.** All dependencies are loaded from CDN; open `index.html` with any static file
server.
...
- **Graph view** — interactive Cytoscape.js graph with multiple layout algorithms (fCoSE, Dagre,
  CoSE, Breadth-first, Grid, Circle)
...
- **Containment** — parent–child relationships shown as edges with filled diamonds or as compound
  (nested) shapes
- **Deep links** — all state (model, filters, drill node, depth, view) is encoded in the URL query
  string; share or bookmark any view
