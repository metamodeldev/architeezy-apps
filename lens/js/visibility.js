...
/**
 * Pure: counts visible relationships per type. A relation is counted when both source/target element types are active
 * AND (when drillVisibleIds is provided) both endpoints are in the drill scope.
 *
 * @param {{ allRelations: object[]; elemMap: object; activeElemTypes: Set<string>; drillVisibleIds?: Set<string> }} params
 ...
 *
 * Traversal rules: - Only type-active nodes (plus the root itself) enter the visible set and the frontier. - The root
 * is always traversable even when its type is filtered out. - Edges followed: those in activeRelTypes OR containment
 * edges. - In compound mode, parent↔child links are also traversed.
 *
 * @param {{
 *   rootId: string;
 *   drillDepth: number;
 ...
 * }} params
 * @returns {{ visibleIds: Set<string>; nodeDepth: Map<string, number> }}
 */
export function computeDrillBfs({
  ...
  for (const e of edges) {
    ...
    adj.get(e.source).push({ neighborId: e.target, type: e.type, isContainment: Boolean(e.isContainment) });
    adj.get(e.target).push({ neighborId: e.source, type: e.type, isContainment: Boolean(e.isContainment) });
  }

  // parent → children index for compound traversal
...
/**
 * Counts elements in the current drill scope per type. Uses drillVisibleIds so the count reflects the BFS result
 * (including type filter).
 */
...
/**
 * Refreshes count badges and dim state for element-type filter rows. Dim reflects availability in the current context
 * (drill scope or full model). A type hidden by the user's checkbox is NOT dimmed — it is available, just filtered.
 */
...
/**
 * In compound mode, when a child node is hidden its parent membership must be removed (node.move({ parent: null })) so
 * Cytoscape does not collapse the compound container to zero size. When the child becomes visible again it is restored
 * to its original parent (modelParent data field).
 *
...
/**
 * Applies full-model visibility: hides nodes/edges whose type is not active, then refreshes filter UI and (if in table
 * view) the table.
 */
 ...
 * BFS rules:
 *
 * - Only type-active nodes (plus the drill-root itself) are added to the visible set and used as frontier — nodes
 *   reachable only via filtered-out types are excluded because they have no visible path back to the drill-root.
 * - The drill-root is always traversable and always shown, even if its type is filtered.
 *
 * Edge visibility rule (universal):
 *
 * - An edge is shown only when min(depth[src], depth[tgt]) < drillDepth. This hides "cross-edges" that connect two nodes
 *   both sitting on the outer boundary (depth = drillDepth), which would otherwise clutter the diagram.
 */
