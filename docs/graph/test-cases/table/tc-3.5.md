...
1. In Table, click on a Microservice row
   - Graph shows only Microservices (due to filter), target node is visible and selected
2. If a row is for an entity that is currently filtered out (shouldn't appear in table) - not
   applicable because row wouldn't exist.
...
1. Hover mouse over a table row
   - Row background changes color (e.g., light gray) to indicate it's interactive
...
**Note**: The requirement says "Click on a row in the 'Entities' table." So navigation is defined
for Entities tab. For Relationships tab, it might also be desirable to navigate to either source or
target. Let's define one if applicable.
...
1. Click on the row (or maybe on the Source/Target cells individually)
   - If whole row clickable: view switches to Graph and selects ServiceA (or ServiceB)? Need
     definition.
   - Perhaps only clicking on Source or Target cell links navigates.
   - Assume: clicking on a relationship row selects one of the endpoints (maybe the source) or
     perhaps it selects the relationship itself (highlight edge). Depending on product design, this
     may not be defined.
...
- Graph uses the `dagre` layout algorithm for deterministic positioning
...
1. Click a table row that corresponds to a distant node
   - Camera moves to target node with a smooth animation (not instant jump)
...
1. Click its table row
   - Node is centered (or near-center) in the viewport; node not clipped at edge
   - Zoom level remains unchanged (unless node is very far and zoom-out is needed? Typically
     centering retains zoom)
...
1. Click on a table row for an entity Y that is within current drill-down scope (if drill-down
   active) OR if no drill-down, any visible row.
   - Graph view switches, but:
     - Global filters remain unchanged
     - Drill-down state (if any) remains? Actually, navigating to a node from Table might clear
       drill-down? The spec says: "The system switches the view mode to 'Graph'. The corresponding
       node is selected and highlighted. The camera performs a smooth centering animation on the
       selected node." It doesn't say it clears drill-down. If drill-down was active, navigating to
       a node outside the drill-down scope would be impossible (row wouldn't exist). If within
       scope, drill-down stays.
...
1. Click row for entity ID "node-123"
   - URL updates to include selection parameter? Not required but could be `&select=node-123` to
     allow deep linking to selected node.
   - Check URL: may add `?selected=node-123` or similar
2. Copy URL and open new tab
   - App loads in Graph view with that node selected and centered (if deep linking supports selected
     state)
