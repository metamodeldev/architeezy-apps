...
1. Open the application at `/graph/`
   - **Expected**: The "Select Model" modal appears centered on the screen with a backdrop; the main content area (graph/table) is not visible or is blocked
2. Observe the modal header
   - **Expected**: Modal displays a title like "Select Model" or "Choose a Model"
3. Observe the model list
   - **Expected**: The list displays all available models with their names, icons, and descriptions
...
1. Click on the **e-commerce** model card
   - **Expected**: The modal closes immediately with a smooth animation (if configured)
2. Wait for the data fetch to complete
   - **Expected**: The graph or table view renders with model data; a loading indicator may show during fetch
3. Observe the application header
   - **Expected**: The header displays "e-commerce" as the current model name
4. Verify model data is displayed
   - **Expected**: Nodes/edges (graph) or rows (table) corresponding to the e-commerce model are visible
...
1. Type "e-commerce" into the search input field
   - **Expected**: The model list updates in real-time to show only models matching the query; **e-commerce** is visible, others are hidden
2. Clear the search input
   - **Expected**: All models become visible again
3. Type "xyz" (a query with no matches)
   - **Expected**: A message "No models found" or equivalent is displayed; the list is empty
...
1. Observe the application behavior
   - **Expected**: The model selection modal opens (as per startup priority)
2. Observe the modal content
   - **Expected**: A message "No models available" or similar is displayed; the model list is empty or shows a placeholder state
3. Attempt to interact with the main view
   - **Expected**: Main view remains blocked or displays an empty state; user cannot proceed without selecting a model
...
1. Select **empty-model** from the list
   - **Expected**: Modal closes, data fetch initiates
2. Wait for the graph/table to render
   - **Expected**: An empty state is shown with a message like "This model contains no data" or a placeholder illustration
3. Verify the model name in header
   - **Expected**: Header shows "empty-model"
4. Try to access model selection again
   - **Expected**: The selection interface remains accessible; user can switch to another model
...
1. Press the `Down Arrow` key
   - **Expected**: The first model card receives focus (highlighted)
2. Press `Down Arrow` multiple times
   - **Expected**: Focus moves down through the list of model cards
3. Press `Up Arrow`
   - **Expected**: Focus moves up the list
4. Press `Enter` on a focused model card
   - **Expected**: The modal closes and the selected model loads
5. Press `Esc`
   - **Expected**: The modal closes; if no model was selected, the application remains in its current state or falls back to a default
...
1. Type the letter "e" into the search field
   - **Expected**: Models matching "e" are shown (e-commerce, e-com)
2. Type "-com" (so full query is "e-com")
   - **Expected**: Only **e-com** and **e-commerce** remain visible
3. Type "model" (full query now "e-com-model")
   - **Expected**: Only **test-model** matches (if search logic treats space as AND) or none matches (if it's a continuous string); test accordingly
...
| Models         | e-commerce, e-com, test-model, demo |
| Query 1        | "e" → 2 matches                     |
| Query 2        | "e-com" → 2 matches                 |
| Query 3        | "model" → 1 match (test-model)      |
