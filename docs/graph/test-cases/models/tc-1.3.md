...
- User has previously loaded a model (`e-commerce`) and applied some filters (`type:microservice`)
  and view mode (Graph)
...
2. Reopen the browser and navigate to `/graph/` (without any URL parameters)
   - The application reads from `localStorage`, finds the last session, and automatically begins
     loading the stored model (`e-commerce`)
3. Wait for the data to load
   - The model `e-commerce` appears in the header; the graph view is active with the filter
     `type:microservice` already applied
4. Verify the filter state
   - An active filter badge or indicator shows "type:microservice"; only matching nodes are visible
...
1. Load the application
   - No restoration occurs; the model selection modal opens (as per startup loading priority: URL
     params → localStorage → manual selector)
   - The manual selection interface is presented
   - No model is loaded

### Test Data

| Storage state | empty/cleared | | Expected UI | Select Model modal |
...
- Model `old-model` has been deleted or the user's access has been revoked (API returns 404 or empty
  for this model)
- User opens the app at `/graph/`
...
1. Application attempts to restore from storage
   - System attempts to fetch `old-model` and receives an error (404 or access denied)
2. Observe the behavior
   - The stored reference to `old-model` is cleared from `localStorage`; the model selection modal
     opens
3. Try to select a different model
   - The selector works normally; new selection saves correctly
...
1. Select a model; before the fetch completes, close the browser tab
   - If the fetch fails or is incomplete, the state (model ID, viewState) is NOT saved to
     `localStorage`
2. Reopen the app at `/graph/`
   - No restoration occurs because nothing was saved; the selection modal opens
...
1. While signed out (anonymous), reopen the app
   - If storage still contains the model ID and viewState, restoration will attempt to load it
   - **However**, if the model `model-A` is private and the user is now anonymous, the Access Denied
     flow occurs (TC-1.2.2)
2. Observe the actual behavior
   - Public models can still be restored from storage even when signed out; private models will
     trigger access denial because storage is not tied to account session, only to the browser
...
- Two browser tabs (Tab A and Tab B) are open on the app, both initially showing the same model or
  different models
...
1. In Tab A, load model `A` with filter `x`
   - Tab A saves `{model: A, viewState: {filter: x}}` to `localStorage`
2. In Tab B, load model `B` with filter `y`
   - Tab B saves `{model: B, viewState: {filter: y}}` to `localStorage`
3. Close both tabs and reopen the app (new tab or refreshed)
   - The state from Tab B is restored because it was the last write; the model `B` with filter `y`
     loads
...
1. Refresh the browser or close and reopen the app (with persistence enabled)
   - Model and view state (filters, view mode) are restored, but:
...
2. Verify the restored state
   - Only the "View State" (model ID, filters, mode, drill-down) persists; transient UI like
     zoom/pan/sidebar scroll is not persisted
...
**Business Rule**: "View settings are namespaced by model type. Filters applied to one model
...
1. Load `e-commerce`, apply filter `layer:presentation`
   - Filter is applied; view state saved with namespace for `ApplicationArchitecture` type
2. Switch to model `hr-system` (same type)
   - The filter `layer:presentation` automatically applies to `hr-system` because it's the same type
     (namespace shared)
3. Switch to model `network-diagram` (different type)
   - No filter is applied; the view state for `NetworkDiagram` type is either default or its own
     stored state
4. Verify persistence behavior
   - Switching back to `e-commerce` or `hr-system` restores the `layer:presentation` filter;
     `network-diagram` uses its own separate namespace
