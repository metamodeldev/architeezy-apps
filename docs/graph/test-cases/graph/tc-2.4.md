...
2. Select a different layout algorithm (e.g., "Hierarchical")
   - **Expected**: Nodes smoothly animate to new positions according to the selected algorithm's logic
   - Edges re-route accordingly
   - Layout type indicator updates in UI
3. Verify layout change
   - **Expected**: Hierarchical layout shows nodes in layers/tiers; Force-Directed spreads nodes organically, etc.
...
1. Click and drag a specific node to a new location
   - **Expected**: Node moves with cursor; when released, it stays at that position (becomes "fixed")
2. Observe other nodes
   - **Expected**: They may adjust slightly due to force simulation, but the manually moved node remains near its drop location
3. Switch to a different layout algorithm and back
   - **Expected**: Manual positions are preserved (or optionally reset if design says layout override clears on switch)
...
1. Trigger "Refresh Layout" command (button or menu)
   - **Expected**: The system re-runs the current layout algorithm from scratch
...
2. Verify nodes are reorganized
   - **Expected**: Graph looks tidier, following the layout logic
...
1. Switch layout or refresh
   - **Expected**: Nodes jump to new positions immediately without smooth animation
   - Why: To maintain performance and avoid long animation times
2. Observe performance
   - **Expected**: Operation is fast and responsive; no sluggishness
...
**Related to**: Business rule: "The system recalculates the layout whenever the set of visible entities or relationships changes (e.g., changing filters, containment modes, or entering/exiting Drill-down)."
...
1. Uncheck an entity type ( hides 10 nodes)
   - **Expected**: Layout is recalculated for remaining 20 nodes; graph rearranges
2. Re-check that entity type (shows 10 nodes)
   - **Expected**: Layout recalculates again to accommodate 30 nodes
3. Enter drill-down on a node
   - **Expected**: Layout recalculates for the drill-down scope (smaller subgraph)
4. Exit drill-down
   - **Expected**: Layout recalculates for full graph again
...
1. Change Highlight depth from 1 to 2 (using depth control)
   - **Expected**: Graph updates to show more neighbors in full opacity (or less dimmed)
   - **BUT**: Node positions remain unchanged; no layout recalculation occurs
2. Change depth to 3
   - **Expected**: Same: visual highlight changes, but node positions static
...
1. Switch containment mode to "Compound" (nested nodes)
   - **Expected**: Layout recalculates to position child nodes inside parent boundaries
   - Visual hierarchy changes
2. Switch back to "Edge-based"
   - **Expected**: Layout recalculates to separate nodes with containment edges
...
1. Uncheck the parent's entity type (or filter it out)
   - **Expected**: Parent node disappears
