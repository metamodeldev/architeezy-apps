...
1. Open the export menu in the table toolbar
   - **Expected**: Dropdown appears with "Export to CSV" option
2. Select "Export to CSV"
   - **Expected**: Loading indicator appears if generation exceeds 200ms
3. Wait for download to complete
   - **Expected**: CSV file downloads automatically with name pattern: `{modelName}-entities-{timestamp}.csv`
4. Open the CSV file
   - **Expected**: File contains exactly 4 columns (Name, Type, Status, Owner) in that order
...
1. Click "Export to CSV"
   - **Expected**: CSV contains only Name and Type columns
2. Verify exported columns
   - **Expected**: No Status or Owner columns appear in the file
...
3. Open CSV in a spreadsheet program (Excel, Google Sheets)
   - **Expected**: Data is correctly parsed:
...
2. Open the CSV file
   - **Expected**: File contains only the header row with column names; no data rows
...
1. Export to CSV
   - **Expected**: CSV rows appear in the same order as displayed in the table
2. Verify row order in file
   - **Expected**: Sorting order matches UI exactly
...
1. Export to CSV
   - **Expected**: CSV columns appear in the order: Owner, Name, Type, Status
2. Check header row
   - **Expected**: Column order matches current display order
...
1. Click "Export to CSV" while loading spinner is visible
   - **Expected**: Either:
     ...
     - Export initiates and the generated CSV reflects the data that finishes loading first (race condition handled)
...
1. Click "Export to CSV"
   - **Expected**: Loading indicator appears with a "Cancel" button (as per UX details)
2. Click "Cancel" before generation completes
   - **Expected**: Export process aborts; no file downloaded
3. UI returns to normal state
   - **Expected**: User can interact with table again
...
1. Click "Export to CSV"
   - **Expected**: CSV contains only the 100 visible microservice entities
2. Verify the exported data
   - **Expected**: All rows have type "Microservice"; no other types appear
